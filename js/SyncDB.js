...
SyncDB.Filter.Equal = SyncDB.Filter.Base.extend({
    ...
    index_lookup : function(table) {
        if (table.I[this.field]) {
            ...
            UTIL.error("no index for %s", this.field);
            return false;
        }
    },
...
if (UTIL.App.is_ipad || UTIL.App.is_phone || UTIL.App.has_local_database) {
    SyncDB.KeyValueDatabase = UTIL.Base.extend({
        ...
        _transaction : function(tx) {
            ...
            ea.ready(function() {
                UTIL.log("done in %o ms", new Date() - t);
            });
            ea.start();
        },
        get : function(key, cb) {
    ...
    get : function(cb) {
        ...
        if (!this.value) { // cache this, we will fetch
            if (this.get_queue) {
                ...
                // TODO: really "null", not "this"? What is special here?
                // I made that explicit from a former simple "return;"
                return null;
            }
            this.get_queue = [ cb ];
...
SyncDB.Index = SyncDB.LocalField.extend({
    constructor : function(ls, name, tkey, tid) {
        ...
        this.get(UTIL.make_method(this, function() {
...
SyncDB.CritBitIndex = {
    ...
    lookup_contains : function(index) {
        ...
        var a = [];
        this.m.foreach(function(key, val) {
            a.push(val);
        },
        index.a, index.b);
        return a;
...
SyncDB.Table = UTIL.Base.extend({
    ...
    select : function(filter, callback) {
        ...
        var f = extra.length ? function(error) {
...
SyncDB.MeteorTable = SyncDB.Table.extend({
    constructor : function(name, schema, channel, db) {
        ...
        regtype(this.incoming, "_reply", SyncDB.Meteor.Reply, {
            id : s, 
            rows : new serialization.Array(this.parser_in)
        });
        ...
        */
        ...
        regtype(this.out, "_syncreq", SyncDB.Meteor.SyncReq, {
            ...
            filter : new serialization.Object(Or)
        });
                ...
                this.ls.remove(k, this.M(function(error, value) { // TODO: make useful with different storage errors etc.
                    if (!error) {
                        if (value) {
                            ...
                            callback(new SyncDB.Error.NotFound());
                        }
                    } else {
                        callback(error);
                    }
...
SyncDB.Types = {
    ...
    Base : UTIL.Base.extend({
        ...
        constructor : function(name) {
            ...
            for (i = this.flags.length - 1; i >= 0; i--) {
                for (flag in this.flags[i]) {
                    if (UTIL.functionp(this.flags[i][flag])) {
                        switch (flag) {
                            case "check":
                            case "transform":
                                // these functions are expected to curry
                                this[flag] = this.flags[i][flag](this[flag]);
                                break;
                        }
                    }
                }
            ...
            for (i = 0; i < this.flags.length; i++) {
                for (var sym in this.flags[i]) {
                    if (UTIL.has_prefix(sym, "is_") || UTIL.has_prefix(sym, "get_")) {
    ...
    create_draft : function(row, cb) {
        SyncDB.LocalTable.prototype.insert.call(this, row, this.M(function(err, row_) {
            ...
            if (row[this.schema.key]) {
                this.draft_index.insert(row_[this.schema.key], row [this.schema.key]);
            }
            cb(err, row_);
            return null;
        }));
...
SyncDB.Connector = SyncDB.LocalField.extend({
    ...
    commit : function(key) {
        ...
        this.drafts.select_by(key, this.M(function(error, row) {
            if (error) return this.cb(key, error);
            ...
            var callback = this.M(function(error, row) {
                ...
                if (!error) {
                    ...
                    this.drafts.remove_by(key, this.M(function(err) {
            ...
            if (oid.length) { // corresponds to online entry
                ...
                // update or delete
                if (!row) {
                    this.online.remove_by(oid, callback);
                } else {
                    this.online.select_by(oid, this.M(function (err, orow) {
                        if (err || orow.length != 1) {
                            UTIL.error("Something fishy in Connector#commit: %o", err);
                        }
                        this.online.update(row[0], callback, orow[0]);
                    }));
                }
            } else {
                this.online.insert(row[0], callback);
            }
            return null;
        }));
    }
